...
cprint(colored("Winfetch version ", selected_theme["header"]) + colored(current_version, selected_theme["value"]))

# Print Date and time
cprint(colored("Date: ", selected_theme["header"]) + colored(formatted_date, selected_theme["value"]))
cprint(colored("Time: ", selected_theme["header"]) + colored(formatted_time, selected_theme["value"]))
cprint(colored("It is a ", selected_theme["header"]) + colored(day, selected_theme["value"]))

# Print System info
for i, gpu_info in enumerate(gpu_infos, start=1):
    cprint(colored(f"GPU {i}: ", selected_theme["header"]) + colored(gpu_info, selected_theme["value"]))

cprint(colored("CPU: ", selected_theme["header"]) + colored(cpu, selected_theme["value"]))

# Get CPU cores
cpu_cores = sum(int(cpu.NumberOfCores) for cpu in computer.Win32_Processor())
cprint(colored("CPU cores: ", selected_theme["header"]) + colored(cpu_cores, selected_theme["value"]))
